...
export default class GetListOfListsDemoWebPart extends BaseClientSideWebPart<IGetListOfListsDemoWebPartProps> {
 ...
 private _renderListOfLists(items: ISharepointList[]):void {
  ...
  items.forEach((item:ISharepointList)=>{
    html += `<ul class="${styles.list}"><li class="${styles.listItem}"><span class="ms-font-l">${item.Title}</span></li><li class="${styles.listItem}"><span class="ms-font-l">${item.Id}</span></li></ul>`;
  });
  ...
  public render(): void {
    ...
    <section class="${styles.getListOfListsDemo} ${!!this.context.sdks.microsoftTeams ? styles.teams : ''}">
